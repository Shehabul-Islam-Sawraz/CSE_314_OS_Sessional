...
void ReaderLock(struct read_write_lock * rw) {
    ...
    while(rw->writer_threads > 0 || rw->waiting_writer_threads > 0){
...
void WriterLock(struct read_write_lock * rw) {
    ...
    while(rw->writer_threads > 0 || rw->reader_threads > 0) {
...
void WriterUnlock(struct read_write_lock * rw) {
    ...
    // If there are any writer threads, we wake them up
    if(rw->waiting_writer_threads > 0) {
        pthread_cond_signal(&rw->cond_write_wait);
    }
    // Otherwise, if there are any waiting reader threads, 
    // we wake up all the reading threads 
    else if(rw->waiting_reader_threads > 0) {
